...
//    [self searchActivities:searchBar.text];
    [self searchDealerships:searchBar.text];
}




- (void)searchDealerships:(NSString *)query {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@&api_key=%@&zipcode=%@", EDMUNDSURI, EDMUNDSAPIKEY, encodedQuery]];
    NSLog(@"%@",url);
    // NSLog(encodedQuery);
    ...
    if ((response.expectedContentLength == NSURLResponseUnknownLength) ||
        ...
        (!result))
    {
        NSLog(@"Edmunds: No dealerships found");
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No dealerships found"
    ...
    NSArray *resultsArray = [[jsonParser objectWithString:resultsString error:nil] objectForKey:@"dealerHolder"];
    ...
    {
        Dealer *newDealer = [[Dealer alloc] init];

        newDealer.title = [NSString stringWithFormat:@"%@",[result objectForKey:@"name"]];
        // newDealer.title = newDealer.title;

        newDealer.url = [NSString stringWithFormat:@"%@",[[[[result objectForKey:@"contactinfo"] objectForKey:@"dealer_website"] stringByReplacingOccurrencesOfString:@"&amp;" withString:@"&"] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        
        newDealer.location = [NSString stringWithFormat:@"%@",[[result objectForKey:@"address"] objectForKey:@"city"]];

        newDealer.coordinate = CLLocationCoordinate2DMake([[[result objectForKey:@"address"] objectForKey:@"latitude"] doubleValue], [[[result objectForKey:@"address"] objectForKey:@"longitude"] doubleValue]);
        // NSLog(@"%f",newDealer.coordinate.longitude);
        
        // Add location object to an array
        [dealerArray addObject:newDealer];
    }
    
    // Add locations to map
    [_mapView addAnnotations:dealerArray];
...
-(void) mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control {
    ...
    Dealer *dealer = (Dealer *)view.annotation;

    detail.titleForDealer = dealer.title;
    detail.locationForDealer = dealer.location;
    detail.urlForDealer = dealer.url;
    
    [self.navigationController pushViewController: detail animated:YES];
}

- (MKAnnotationView *)mapView:(MKMapView *)theMapView viewForAnnotation:(Dealer *)annotation
{
    ...
    if (pinView == nil)
    {
        ...
        customPinView.image = [UIImage imageNamed:[NSString stringWithFormat:@"edmunds32x39.png"]];
